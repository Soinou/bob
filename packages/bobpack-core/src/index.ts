...
class Builder {
    ...
    /**
     * The used mini-css-extract-plugin
     */
    private cssExtractPlugin: any;
    ...
    public serve(port: number, hmrPort: number, publicPath: string) {
        const { serve } = safeRequire("@bobpack/serve");
    ...
    public modules(...loaders: Array<"css" | "handlebars" | "pug" | "sass" | "stylus" | "typescript" | "vue">) {
        ...
        loaders.forEach(loader => {
            switch (loader) {
                case "css":
                    const { css, cssExtractPlugin } = safeRequire("@bobpack/css");
                    rules.push(css(this.production));
                    extensions.push(".css");
                    if (this.cssExtractPlugin == null) {
                        this.cssExtractPlugin = cssExtractPlugin;
                    }
                    break;
                case "handlebars":
                    const { handlebars } = safeRequire("@bobpack/handlebars");
                    rules.push(handlebars());
                    extensions.push(".hbs");
                    break;
                case "pug":
                    const { pug } = safeRequire("@bobpack/pug");
                    rules.push(pug());
                    extensions.push(".pug");
                    break;
                case "sass":
                    const { sass, sassExtractPlugin } = safeRequire("@bobpack/sass");
                    rules.push(sass(this.production));
                    extensions.push(".sass", ".scss");
                    if (this.cssExtractPlugin == null) {
                        this.cssExtractPlugin = sassExtractPlugin;
                    }
                    break;
                case "stylus":
                    const { stylus, stylusExtractPlugin } = safeRequire("@bobpack/stylus");
                    rules.push(stylus(this.production));
                    extensions.push(".styl", ".stylus");
                    if (this.cssExtractPlugin == null) {
                        this.cssExtractPlugin = stylusExtractPlugin;
                    }
                    break;
                case "typescript":
                    const { typescript } = safeRequire("@bobpack/typescript");
                    rules.push(typescript(this.parallel));
                    extensions.push(".ts", ".tsx");
                    break;
                case "vue":
                    const { vue, vuePlugin } = safeRequire("@bobpack/vue");
                    rules.push(vue());
                    extensions.push(".vue");
                    this.alias("vue$", "vue/dist/vue.common.js");
                    this.configuration.plugins.push(vuePlugin());
                    break;
                default:
                    break;
            }
    ...
    public clean(directory: string, exclude: string[] = []) {
        const { clean } = safeRequire("@bobpack/clean");
    ...
    public html(template: string) {
        const { html } = safeRequire("@bobpack/html");
